function sortData(data, method, ascending = true){
	...
	else if (method=="shelf" && ascending == true){
		data.sort(function(a,b){
			a = a.shelf_id;
			b = b.shelf_id;
			return a < b ? -1 : a > b ? 1 : 0;
	...
	else if (method=="shelf" && ascending == false){
		data.sort(function(a,b){
			a = a.shelf_id;
			b = b.shelf_id;
			return a < b ? 1 : a > b ? -1 : 0;
...
function setUpSorters(){
	...
	$("#shelf_asc").click(function(){
		if(!$(this).is(activeSorter)){
			sortData(product_data, "shelf", true);
			if(activeSorter!==undefined) switchActive(activeSorter);
			activeSorter = $("#shelf_asc").toggleClass("fa-arrow-circle-up fa-arrow-circle-o-up");
		}

	});
	$("#shelf_desc").click(function(){
		if(!$(this).is(activeSorter)){
			sortData(product_data, "shelf", false);
			if(activeSorter!==undefined) switchActive(activeSorter);
			activeSorter = $("#shelf_desc").toggleClass("fa-arrow-circle-down fa-arrow-circle-o-down");
		}
	});
}
